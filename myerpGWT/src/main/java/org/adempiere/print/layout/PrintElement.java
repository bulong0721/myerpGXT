...
public abstract class PrintElement implements ImageObserver {
	...
	protected Point2D.Double getAbsoluteLocation(Point2D pageStart) {
		Point2D.Double retValue = new Point2D.Double(pageLocation.x + pageStart.getX(), pageLocation.y + pageStart.getY());
		return retValue;
	}
	...
	public String toString() {
		...
		sb.append("Bounds=").append(getBounds()).append(",Height=").append(p_height).append("(").append(p_maxHeight).append("),Width=")
				.append(p_width).append("(").append(p_maxHeight).append("),PageLocation=").append(pageLocation);
		sb.append("]");
		return sb.toString();
	} // toString
}
