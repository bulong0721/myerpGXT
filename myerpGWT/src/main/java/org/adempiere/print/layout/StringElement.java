...
public class StringElement extends PrintElement {
	...
	public StringElement(String inText, Font font, Paint paint, boolean translateText) {
		...
		if (translateText) {
			...
			if (count > 0 && count % 2 == 0) {
				...
				inText = MsgUtil.parseTranslation(EnvUtil.getCtx(), this.originalString);
			}
		}
		...
		for (int i = 0; i < lines.length; i++) {
			...
			if (i == 0) {
				...
				this.stringView[i].addAttribute(TextAttribute.UNDERLINE, TextAttribute.UNDERLINE_LOW_ONE_PIXEL, 0, endIndex);
			} else
		...
		for (int i = 0; i < lines.length; i++) {
						...
						// Attributed String
			{
				...
				if (endIndex > startIndex) {
					...
					this.stringView[i].addAttribute(TextAttribute.UNDERLINE, TextAttribute.UNDERLINE_LOW_ONE_PIXEL, startIndex, endIndex);
				}
			} else
				this.stringView[i] = this.stringPaper[i];
		}
			...
			// Size Limits
				...
				while (measurer.getPosition() < iter.getEndIndex()) {
					...
					if (p_maxHeight == 0f || (height + lineHeight) <= p_maxHeight) {
						...
						} else if ((PrintFormatItem.FIELDALIGNMENTTYPE_TrailingRight.equals(fieldAlignmentType) && layout.isLeftToRight())
								|| (PrintFormatItem.FIELDALIGNMENTTYPE_LeadingLeft.equals(fieldAlignmentType) && !layout.isLeftToRight()))
						...
						else if (PrintFormatItem.FIELDALIGNMENTTYPE_Block.equals(fieldAlignmentType)
	...
	protected boolean calculateSize() {
		...
		if (fieldAlignmentType != null) {
			...
			if (changed)
				System.out.println("StringElement.calculate size - Width=" + p_width + "(" + p_maxWidth + ") - Height=" + p_height + "("
						+ p_maxHeight + ")");
		}
		return true;
	}
