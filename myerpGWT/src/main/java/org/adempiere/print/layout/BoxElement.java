...
public class BoxElement extends PrintElement {
	 ...
	 *            color
	 */
	public BoxElement(PrintFormatItem item, Color color) {
		super();
	...
	private boolean isTypeBox(PrintFormatItem item) {
		return item.getPrintFormatType().equals(PrintFormatItem.PRINTFORMATTYPE_Line)
				|| item.getPrintFormatType().equals(PrintFormatItem.PRINTFORMATTYPE_Rectangle);
	}

	/** The Item */
	private PrintFormatItem	m_item	= null;
	...
	public void paint(Graphics2D g2D, int pageNo, Point2D pageStart, boolean isView) {
		...
		if (m_item.getPrintFormatType().equals(PrintFormatItem.PRINTFORMATTYPE_Line))
			...
			if (m_item.isFilledRectangle()) {
				if (type.equals(PrintFormatItem.SHAPETYPE_3DRectangle))
					g2D.fill3DRect(x, y, width, height, true);
				else if (type.equals(PrintFormatItem.SHAPETYPE_Oval))
					g2D.fillOval(x, y, width, height);
				else if (type.equals(PrintFormatItem.SHAPETYPE_RoundRectangle))
