 ...
 * 
 * @author Teo Sarca, SC ARHIPAC SERVICE SRL <li>BF [ 1888085 ] One line
 *         location field not working <li>BF [ 1888094 ] PF: label & label
 *         suffix not working for location field <li>BF [ 2695078 } Country is
 *         not translated on invoice
 * 
 * @author Michael Judd (Akuna Ltd) <li>BF [ 2695078 ] Country is not translated
 *         on invoice
 */
public class LocationElement extends GridElement {
	 ...
	 * @param labelSuffix
	 */
	public LocationElement(int C_Location_ID, Font font, Paint color, boolean isHeightOneLine, String label,
			String labelSuffix, String language) {
		super(isHeightOneLine ? 1 : 10, 1); // max
		setGap(0, 0);
		PrintLocation ml = PrintUtil.getPrintLocation(C_Location_ID);
		// log.fine("C_Location_ID=" + C_Location_ID);
		if (ml != null) {
			int index = 0;
			if (isHeightOneLine) {
				String line = (label != null ? label : "") + ml.toString() + (labelSuffix != null ? labelSuffix : "");
