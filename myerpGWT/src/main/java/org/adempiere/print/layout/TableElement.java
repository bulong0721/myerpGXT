...
public class TableElement extends PrintElement {
	private static CLogger	log	= CLogger.getCLogger(TableElement.class);
	 ...
	 */
	public TableElement(ValueNamePair[] columnHeader, int[] columnMaxWidth, int[] columnMaxHeight, String[] columnJustification,
			boolean[] fixedWidth, ArrayList<Integer> functionRows, boolean multiLineHeader, Object[][] data, KeyNamePair[] pk,
			String pkColumnName, int pageNoStart, Rectangle firstPage, Rectangle nextPages, int repeatedColumns,
			HashMap<Integer, Integer> additionalLines, HashMap<Point, Font> rowColFont, HashMap<Point, Color> rowColColor,
			HashMap<Point, Color> rowColBackground, PrintTableFormat tFormat, ArrayList<Integer> pageBreak, boolean[] colSuppressRepeats) {
		super();
	...
	/** Format of Table */
	private PrintTableFormat			m_tFormat;
	/** Page Break Rows */
		...
		for (int dataCol = 0; dataCol < cols; dataCol++) {
			...
			for (int row = 0; row < rows; row++) {
				...
				if (dataItem instanceof Boolean) {
					...
					dataSizes[row][col].addBelow(new Dimension((int) ((ImageElement) dataItem).getWidth(), (int) ((ImageElement) dataItem)
							.getHeight()));
	...
	private void distributeColumns(int availableWidth, int fromCol, int toCol) {
		...
		for (int x = 0; remainingWidth > 0 && x < 5; x++) {
	 ...
	 * @param isView true if online view (IDs are links)
	 */
	private void printColumn(Graphics2D g2D, int col, final int origX, final int origY, boolean leftVline, final int firstRow,
			final int nextPageRow, boolean isView) {
		int curX = origX;
		...
		if (leftVline) // draw left | line
		{
			...
			if (m_tFormat.isPaintBoundaryLines()) // -> | (left)
				g2D.drawLine(origX, (int) (origY + m_tFormat.getLineStroke().floatValue()), origX,
		...
		if (!bg.equals(Color.white)) {
			g2D.setPaint(bg);
			g2D.fillRect(curX, (int) (curY - m_tFormat.getLineStroke().floatValue()), (int) (colWidth - m_tFormat.getVLineStroke()
					.floatValue()), (int) (rowHeight - (4 * m_tFormat.getLineStroke().floatValue())));
		}
		...
		// if (m_columnHeader[col].toString().length() > 0)
		{
			...
			while (measurer.getPosition() < iter.getEndIndex()) // print header
			{
				...
				if (iter.getEndIndex() != measurer.getPosition())
					fastDraw = false;
				float lineHeight = layout.getAscent() + layout.getDescent() + layout.getLeading();
				if (alignment.equals(PrintFormatItem.FIELDALIGNMENTTYPE_Block)) {
					layout = layout.getJustifiedLayout(netWidth + 2);
					fastDraw = false;
				}
				curY += layout.getAscent();
				float penX = curX;
				if (alignment.equals(PrintFormatItem.FIELDALIGNMENTTYPE_Center))
					penX += (netWidth - layout.getAdvance()) / 2;
				else if ((alignment.equals(PrintFormatItem.FIELDALIGNMENTTYPE_TrailingRight) && layout.isLeftToRight())
						|| (alignment.equals(PrintFormatItem.FIELDALIGNMENTTYPE_LeadingLeft) && !layout.isLeftToRight()))
		...
		if (m_tFormat.isPaintVLines()) // -> | (right)
			g2D.drawLine(curX, (int) (origY + m_tFormat.getLineStroke().floatValue()), curX, (int) (origY + rowHeight - (4 * m_tFormat
					.getLineStroke().floatValue())));
		curX += m_tFormat.getVLineStroke().floatValue();
		// X end line
		if (m_tFormat.isPaintHeaderLines()) {
			g2D.setPaint(m_tFormat.getHeaderLine_Color());
		...
		for (int row = firstRow; row < nextPageRow; row++) {
			...
			if (!bg.equals(Color.white)) {
				g2D.setPaint(bg);
				g2D.fillRect(curX, curY, (int) (colWidth - m_tFormat.getVLineStroke().floatValue()), (int) (rowHeight - m_tFormat
						.getLineStroke().floatValue()));
			}
			curX += H_GAP; // upper left gap
			curY += V_GAP;
							...
							for (int lineNo = 0; lineNo < lines.length; lineNo++) {
																					...
																					// element
								{
									...
									if ((m_columnMaxHeight[col] <= 0 || (usedHeight + lineHeight) <= m_columnMaxHeight[col])
										...
										if (alignment.equals(PrintFormatItem.FIELDALIGNMENTTYPE_Center))
											penX += (netWidth - layout.getAdvance()) / 2;
										else if ((alignment.equals(PrintFormatItem.FIELDALIGNMENTTYPE_TrailingRight) && layout
												.isLeftToRight())
												|| (alignment.equals(PrintFormatItem.FIELDALIGNMENTTYPE_LeadingLeft) && !layout
														.isLeftToRight()))
											penX += netWidth - layout.getAdvance();
										//
