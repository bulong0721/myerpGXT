...
/**
 * Adempiere Paper Change log:
 * <ul>
 * <li>2009-02-10 - armen - [ 2580531 ] Custom Paper Support - https://sourceforge
 * .net/tracker2/?func=detail&atid=879335&aid=2580531&group_id=176962
 * </ul>
 * 
 * @author Jorg Janke
...
public class CPaper extends Paper {
    ...
    public CPaper(PageFormat pf){
        ...
        MediaSize ms = null;
        if (msn == null) msn = MediaSize.findMedia(y / 72, x / 72, MediaSize.INCH); // flip
                                                                                    // it
        if (msn != null) ms = MediaSize.getMediaSizeForName(msn);
    ...
    public void setMediaSize(MediaSize mediaSize, boolean landscape) {
        if (mediaSize == null) throw new IllegalArgumentException("MediaSize is null");
    ...
    public void setMediaSize(double x, double y, int units, boolean landscape) {
        if (x == 0 || y == 0) throw new IllegalArgumentException("MediaSize is null");
    ...
    public MediaSizeName getMediaSizeName() {
        // AA Goodwill: Custom Paper Support
        if (m_mediaSize == null) return MediaSizeName.ISO_A4;
        // End Of AA Goodwill
    ...
    public MediaPrintableArea getMediaPrintableArea() {
        MediaPrintableArea area = new MediaPrintableArea((float) getImageableX() / 72, (float) getImageableY() / 72,
                                                         (float) getImageableWidth() / 72,
                                                         (float) getImageableHeight() / 72, MediaPrintableArea.INCH);
        // log.fine( "CPaper.getMediaPrintableArea",
        // area.toString(MediaPrintableArea.INCH, "\""));
        return area;
    ...
    public void setMediaPrintableArea(MediaPrintableArea area) {
        ...
        setImageableArea(area.getX(inch) * 72, area.getY(inch) * 72, area.getWidth(inch) * 72,
                         area.getHeight(inch) * 72);
    ...
    public PrintRequestAttributeSet getPrintRequestAttributeSet() {
        ...
        // media-printable-area = (25.4,25.4)->(165.1,228.6)mm - class
        // javax.print.attribute.standard.MediaPrintableArea
        pratts.add(getMediaPrintableArea());
        // orientation-requested = landscape - class
        // javax.print.attribute.standard.OrientationRequested
        if (isLandscape()) pratts.add(OrientationRequested.LANDSCAPE);
        else pratts.add(OrientationRequested.PORTRAIT);
        // media = na-legal
    ...
    public void setPrintRequestAttributeSet(PrintRequestAttributeSet prats) {
        ...
        for (int i = 0; i < atts.length; i++) {
            if (atts[i] instanceof OrientationRequested) {
                ...
                if (or.equals(OrientationRequested.PORTRAIT)) landscape = false;
                else landscape = true;
    ...
    public PageFormat getPageFormat() {
        ...
        if (m_landscape) orient = PageFormat.LANDSCAPE;
    ...
    public String toString() {
        ...
        sb.append(getWidth() / 72).append("x").append(getHeight() / 72).append('"').append(m_landscape ? " Landscape " : " Portrait ").append("x=").append(getImageableX()).append(",y=").append(getImageableY()).append(" w=").append(getImageableWidth()).append(",h=").append(getImageableHeight()).append("]");
        return sb.toString();
    } // toString
    ...
    public String toString(Properties ctx) {
        ...
        if (m_mediaSize != null) {
            ...
            if (!name.startsWith("iso")) sb.append(" - ").append(m_mediaSize.toString(MediaSize.INCH, "\"")).append(" (").append(getMediaPrintableArea().toString(MediaPrintableArea.INCH,
                                                                                                                                                                  "\""));
            if (!name.startsWith("na")) sb.append(" - ").append(m_mediaSize.toString(MediaSize.MM, "mm")).append(" (").append(getMediaPrintableArea().toString(MediaPrintableArea.MM,
                                                                                                                                                               "mm"));
            // Print Orientation
            sb.append(") - ").append(MsgUtil.getMsg(ctx, m_landscape ? "Landscape" : "Portrait"));
    ...
    public boolean equals(Object obj) {
        if (obj instanceof CPaper) {
            ...
            if (cp.isLandscape() != m_landscape) return false;
            // media size is more descriptive
            if (getImageableX() == cp.getImageableX() && getImageableY() == cp.getImageableY()
                && getImageableWidth() == cp.getImageableWidth() && getImageableHeight() == cp.getImageableHeight()) return true;
        }
        return false;
    } // equals
    ...
    public double getWidth(boolean orientationCorrected) {
        if (orientationCorrected && m_landscape) return super.getHeight();
    ...
    public double getHeight(boolean orientationCorrected) {
        if (orientationCorrected && m_landscape) return super.getWidth();
    ...
    public double getImageableY(boolean orientationCorrected) {
        if (orientationCorrected && m_landscape) return super.getImageableX();
    ...
    public double getImageableX(boolean orientationCorrected) {
        if (orientationCorrected && m_landscape) return super.getImageableY();
    ...
    public double getImageableHeight(boolean orientationCorrected) {
        if (orientationCorrected && m_landscape) return super.getImageableWidth();
    ...
    public double getImageableWidth(boolean orientationCorrected) {
        if (orientationCorrected && m_landscape) return super.getImageableHeight();
    ...
    public Insets getMargin(boolean orientationCorrected) {
        return new Insets(
                          (int) getImageableY(orientationCorrected), // top
