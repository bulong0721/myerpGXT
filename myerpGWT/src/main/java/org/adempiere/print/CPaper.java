...
public class CPaper extends Paper {
	...
	public CPaper(PageFormat pf) {
		...
		MediaSize ms = null;
		if (msn == null)
			msn = MediaSize.findMedia(y / 72, x / 72, MediaSize.INCH); // flip
																		// it
		if (msn != null)
	 ...
	 * @param y the value to which to set this <code>Paper</code> object's
	 *            height
	...
	public String toString() {
		...
		sb.append(getWidth() / 72).append("x").append(getHeight() / 72).append('"').append(m_landscape ? " Landscape " : " Portrait ")
				.append("x=").append(getImageableX()).append(",y=").append(getImageableY()).append(" w=").append(getImageableWidth())
				.append(",h=").append(getImageableHeight()).append("]");
		return sb.toString();
	} // toString
	...
	public String toString(Properties ctx) {
		...
		if (m_mediaSize != null) {
			...
			if (!name.startsWith("iso"))
				sb.append(" - ").append(m_mediaSize.toString(MediaSize.INCH, "\"")).append(" (")
						.append(getMediaPrintableArea().toString(MediaPrintableArea.INCH, "\""));
			if (!name.startsWith("na"))
				sb.append(" - ").append(m_mediaSize.toString(MediaSize.MM, "mm")).append(" (")
						.append(getMediaPrintableArea().toString(MediaPrintableArea.MM, "mm"));
			// Print Orientation
			sb.append(") - ").append(MsgUtil.getMsg(ctx, m_landscape ? "Landscape" : "Portrait"));
		} else
			sb.append("Custom - ").append(toString());
		// End Of AA Goodwill
		return sb.toString();
	} // toString
