...
public class TableCreateColumns extends ServerProcess {
    ...
    protected String doIt() throws Exception {
        ...
        try {
            ...
            if (putAllTables) addTable(md, catalog, schema);
            else {
    ...
    private void addTableColumn(ResultSet rs, ADTable table) throws Exception {
        ...
        while (rs.next()) {
            ...
            if (columnName.equalsIgnoreCase(tableName + "_ID")) {
            ...
            } else if (columnName.toUpperCase().endsWith("_ACCT")) column.setADReferenceID(DisplayType.Account);
            else if (columnName.equalsIgnoreCase("C_Location_ID")) column.setADReferenceID(DisplayType.Location);
            else if (columnName.equalsIgnoreCase("M_AttributeSetInstance_ID")) column.setADReferenceID(DisplayType.PAttribute);
            else if (columnName.equalsIgnoreCase("SalesRep_ID")) {
                column.setADReferenceID(DisplayType.Table);
                column.setADReferenceValueID(190);
            } else if (columnName.toUpperCase().endsWith("_ID")) column.setADReferenceID(DisplayType.TableDir);
            else if (dataType == Types.DATE || dataType == Types.TIME || dataType == Types.TIMESTAMP
                     || columnName.equalsIgnoreCase("Created") || columnName.equalsIgnoreCase("Updated")) column.setADReferenceID(DisplayType.DateTime);
            ...
            } else if (dataType == Types.CLOB) column.setADReferenceID(DisplayType.TextLong);
                ...
                if (typeName.startsWith("N")) size /= 2;
                if (size > 255) column.setADReferenceID(DisplayType.Text);
                else column.setADReferenceID(DisplayType.String);
            } else if (dataType == Types.INTEGER || dataType == Types.SMALLINT || dataType == Types.DECIMAL
                       || dataType == Types.NUMERIC) {
                if (size == 10) column.setADReferenceID(DisplayType.Integer);
                else column.setADReferenceID(DisplayType.Number);
            } else column.setADReferenceID(DisplayType.String);
